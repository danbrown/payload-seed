...
const validateLocalizationTransform = (
  hook: string,
  value,
  req: PayloadRequest
) => {
  if (
    ...
    throw new Error(
      "Locale transformation should happen before hook is called"
    );
  }
  return value;
};
  ...
  fields: [
    {
      ...
      hooks: {
        beforeValidate: [
          ({ value, req }) =>
            validateLocalizationTransform("beforeValidate", value, req),
        ],
        beforeChange: [
          ({ value, req }) =>
            validateLocalizationTransform("beforeChange", value, req),
        ],
        afterChange: [
          ({ value, req }) =>
            validateLocalizationTransform("afterChange", value, req),
        ],
        afterRead: [
          ({ value, req }) =>
            validateLocalizationTransform("afterRead", value, req),
        ],
      },
    },
    {
